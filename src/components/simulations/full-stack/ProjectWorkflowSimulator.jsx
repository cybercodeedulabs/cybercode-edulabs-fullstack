...
import { Code, GitBranch, UploadCloud, CheckCircle, Trophy } from "lucide-react";

export default function ProjectWorkflowSimulator() {
  const [stage, setStage] = useState(0);

  const steps = [
    { icon: <Code />, title: "Develop Your App", desc: "Write and test your full-stack app locally using React, Express, and MongoDB." },
    { icon: <GitBranch />, title: "Push to GitHub", desc: "Commit and push your source code to a public or private GitHub repository." },
    { icon: <UploadCloud />, title: "Deploy Frontend + Backend", desc: "Host frontend on Netlify and backend on Render — connect via live API URL." },
    { icon: <CheckCircle />, title: "Submit for Review", desc: "Share your GitHub repo and live links for project evaluation." },
    { icon: <Trophy />, title: "Project Approved!", desc: "Congratulations! You’ve completed your Full-Stack Capstone Project." }
  ];
    ...
    <div className="p-6 mt-8 bg-gradient-to-r from-green-50 to-emerald-50 dark:from-gray-900 dark:to-gray-800 rounded-2xl border border-green-300 dark:border-green-700 shadow-lg">
      ...
      <div className="flex flex-wrap justify-center gap-6 mb-8">
        {steps.map((s, i) => (
          ...
          >
            <div className="text-green-600 dark:text-green-400 text-3xl mb-2">{s.icon}</div>
            <p className="font-semibold text-green-700 dark:text-green-300 text-sm mb-1">{s.title}</p>
      ...
      <div className="flex justify-center gap-4">
        <button
          ...
          className={`px-5 py-2 rounded-lg font-medium ${
            stage === 0 ? "bg-gray-400 text-white cursor-not-allowed" : "bg-green-600 hover:bg-green-700 text-white"
          }`}
        >
          ← Previous
        </button>
        <button
          ...
          className={`px-5 py-2 rounded-lg font-medium ${
            stage === steps.length - 1 ? "bg-gray-400 text-white cursor-not-allowed" : "bg-green-600 hover:bg-green-700 text-white"
          }`}
        >
          Next →
        </button>
