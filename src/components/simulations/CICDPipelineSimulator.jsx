...
export default function CICDPipelineSimulator() {
  ...
  const remediationHints = {
    secretInCode:
      "Secrets found in code — move to secrets manager and rotate secrets.",
    outdatedDep:
      "Outdated dependency with known CVE — update or apply mitigation.",
    insecureContainer:
      "Container has high-risk findings — rebuild image with minimal base and reprotect credentials.",
  };
    ...
    <div className="max-w-3xl mx-auto p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-md border border-gray-200 dark:border-gray-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
          ...
          <div className="mt-4">
            <div className="font-semibold mb-2 text-sm">
              Introduce Misconfiguration (simulate real-world issues)
            </div>
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={misconfig.secretInCode}
                onChange={() =>
                  setMisconfig((m) => ({
                    ...m,
                    secretInCode: !m.secretInCode,
                  }))
                }
              />
              Secret in code
            ...
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={misconfig.outdatedDep}
                onChange={() =>
                  setMisconfig((m) => ({
                    ...m,
                    outdatedDep: !m.outdatedDep,
                  }))
                }
              />
            ...
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={misconfig.insecureContainer}
                onChange={() =>
                  setMisconfig((m) => ({
                    ...m,
                    insecureContainer: !m.insecureContainer,
                  }))
                }
              />
          ...
          <div className="mt-4 flex gap-2">
            ...
            <button onClick={reset} className="px-4 py-2 rounded bg-gray-100">
              Reset
            </button>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            {[
              ...
              // ✅ Smarter detection for passed/failed
              const stepPassed = logs.some(
                (l) =>
                  l.toLowerCase().includes(step.label.toLowerCase().split(" ")[0]) &&
                  l.toLowerCase().includes("passed")
              );

              const stepFailed = logs.some(
                (l) =>
                  l.toLowerCase().includes(step.label.toLowerCase().split(" ")[0]) &&
              ...
              const isRunning =
                currentStep &&
                currentStep.toLowerCase().includes(step.label.toLowerCase().split(" ")[0]);

              const status = stepPassed
                ? "passed"
                : stepFailed
                ? "failed"
                : isRunning
            ...
            {result ? (
              result.success ? (
                <div className="text-green-600 font-semibold">
                  Pipeline Success — ready for promotion
                </div>
              ) : (
              ...
              <div className="text-gray-600">
                Run the pipeline to see results and remediation hints.
              </div>
            )}

            {!result?.success &&
              Object.keys(misconfig).some((k) => misconfig[k]) && (
                <div className="mt-2 text-xs text-yellow-700">
                  <div className="font-semibold">Remediation Hints</div>
                  <ul className="list-disc pl-4">
                    {Object.keys(misconfig)
                      .filter((k) => misconfig[k])
                      .map((k) => (
                        <li key={k}>{remediationHints[k]}</li>
                      ))}
                  </ul>
                </div>
              )}
